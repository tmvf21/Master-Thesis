...
def create_rnn_model(n_layers, units, lr, seq_len, n_features) -> Sequential:
    model = Sequential()
    model.add(layers.Input(shape=(seq_len, n_features)))

    for i in range(n_layers):
        model.add(layers.SimpleRNN(units=units, return_sequences=(i != n_layers - 1)))
...
def tune_model_cv(X_train, y_train, max_trials=30):
    ...
    def objective(hps_list):
        ...
        model = create_rnn_model(n_layers=n_layers, 
                                  units=units, 
                                  lr=learning_rate, 
...
if __name__ == '__main__':
    ...
    tuned_model = create_rnn_model(n_layers=best_hps.get('num_layers'),
